...
pPLA PLA;
{
	...
	if (cube.part_size[var] < 0) {
		...
		for(i = first; i <= last; i++)
			...
			PLA->label[i] = strsav(token);	/* add new label */
...
OUT pPLA *PLA_return;
{
	...
	for(i = 0; i < cube.part_size[second]; i++) {
		...
		strsav(PLA->label[i + cube.first_part[third]]);
	}
